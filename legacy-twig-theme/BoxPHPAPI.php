...
class Box_API
{
	...
	public function write_token($token, $type = 'file')
	{
		...
		if (isset($array['error'])) {
			...
			if ($type == 'file') {
				$fp = fopen(get_template_directory() . '/token.box', 'w');
	...
	{
		if ($type == 'file' && file_exists(get_template_directory() . '/token.box')) {
			$fp = fopen(get_template_directory() . '/token.box', 'r');
			$content = fread($fp, filesize(get_template_directory() . '/token.box'));
			fclose($fp);
		} else {
			return false;
		}
